...
function injectGlobalComponents() {
    if(document.getElementById('champ-global-ui-wrapper')) return; // Prevent duplicates
    ...
    globalStyle.innerHTML = `
        ...
        .fab-btn { width: 52px; height: 52px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 22px; color: white; box-shadow: 0 8px 20px rgba(0,0,0,0.2); cursor: pointer; border: 2px solid rgba(255,255,255,0.2); position: fixed; z-index: 1000; transition: transform 0.2s; }
        ...
        .ai-window { position: fixed; bottom: 95px; right: 20px; width: 340px; height: 480px; background: var(--card-bg, white); border-radius: 24px; box-shadow: 0 15px 40px rgba(0,0,0,0.2); z-index: 2998; display: none; flex-direction: column; overflow: hidden; border: 1px solid var(--border-color, #e2e8f0); animation: popIn 0.2s ease-out; }
    ...
    // --- FULL PAGE GOOGLE TRANSLATOR WITH 100+ LANGUAGES ---
    ...
    const transModalHTML = `
        <div id="lang-modal" style="position:fixed; inset:0; background:rgba(9, 9, 11, 0.7); z-index:3005; display:none; align-items:center; justify-content:center; backdrop-filter:blur(8px);">
            <div style="background:var(--card-bg, white); width:90%; max-width:380px; border-radius:24px; overflow:hidden; display:flex; flex-direction:column; max-height:80vh; border: 1px solid var(--border-color, #e2e8f0); box-shadow: 0 20px 50px rgba(0,0,0,0.3);">
                ...
                <div style="padding:15px; border-bottom:1px solid var(--border-color, #e2e8f0); background:var(--bg, #f8fafc);"><input type="text" id="lang-search" placeholder="Search any language..." onkeyup="filterLangs()" style="width:100%; padding:12px; border-radius:12px; border:1px solid var(--border-color, #cbd5e1); outline:none; font-family:inherit; background:var(--input-bg, white); color:var(--text-main, black);"></div>
...
window.sendUserMessage = function() {
    ...
    body.innerHTML += `<div class="msg msg-bot" id="${typingId}"><i class="fas fa-circle-notch fa-spin" style="color:#ec4899;"></i> Analyzing DB...</div>`;
    body.scrollTop = body.scrollHeight;
